...
The easiest way to follow along with your own R studio instance is to click on the binder tag at the top of the screen. It'll open up an Rstudio instance in your browser with everything we need aleady installed.

However if you'd like to follow along on your own computer rather than in your browser, to help ensure that you have all packages you need installed, I've provided a `conda` environment in this repo for your use.
...
rstudio
```
from the terminal where you've activated the environment. The rstudio window that opens will have all required packages already installed, with the exception of the `chunked` and `fst` packages, which have some challenges when installing through conda. To install those packages, please run:
```{R, eval = F}
...
### 1.4 How Does R Work With Memory?
...
#### 1.4.2 Building Blocks of R, Lists and Vectors
...
#### 1.4.3 Variable Assignment and Copy behavior
...
So what do we think will happen when we run the following line? Do you think we'll see something copied here, or no?
```{R, eval=F}
y <- x
```
...
### 1.5.1 Trade Offs When Working With Big Data
...
#### 1.5.2 Types of Memory Challenges
